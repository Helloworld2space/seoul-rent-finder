...
**경계 규칙:** "구·기간"을 바꿀 때만 서버를 다시 부른다(재수집).
보증금·면적·키워드·정렬은 받아둔 데이터 안에서 프론트가 처리한다.
...
### `GET /api/search`
필터 파라미터를 받지 않는다. **구 + 기간**만 받아 원본 전체를 정규화해 내려준다.

요청
```
?districts=11680,11650   // 콤마 구분 법정동코드. 비면 서울 전체
