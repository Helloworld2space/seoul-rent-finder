...
## 11. "오늘의 전/월세 평균" 시세 지도 (today.html)
...
## 14. 동네 리포트 (report.html)

"이 동네는 어떤 곳인가"를 실거래 기록으로 답한다. **추가 API 없이** 이미 수집하는
값(면적·준공년도·전월세 구성)을 더 집계해서 만든다 — 공공 API 호출은 늘지 않는다.

- 판정·표현은 `frontend/profile.js` 순수함수(테스트: `tests/profile.test.js`).
  숫자를 문장으로 바꾸는 자리를 한곳에 모아, **나중에 이 부분만 LLM으로 교체**할 수 있게 했다.
- **표현 원칙**: 동네를 평가·서열화하지 않는다. 관찰된 사실만 중립적으로 전한다
  (특정 지역에 대한 부정적 단정은 분쟁 소지가 있다).
- 거래 10건 미만인 동은 성격을 단정하지 않고 "표본 적음"으로만 표시한다.
- `price_stats`에 `area_avg`·`build_year_avg`·`build_year_count` 컬럼이 필요하다
  (`supabase/price_stats_profile.sql`). 준공년도는 약 2% 결측이라 가중평균 분모를 따로 둔다.
- 데이터는 `/api/prices?district=` 를 그대로 쓴다(새 엔드포인트 없음).
